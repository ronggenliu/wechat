...
public class ImageUtil {
	...
	/**
	 * 读取图片的宽高 File file = new File("E:/pics/1.jpg"); Map<String, Integer> ImgInfo = getImageInfo(file); System.out.println(ImgInfo);
	 */
	public static Map<String, Integer> getImageInfo(File file) throws IOException {

		if (!file.exists()) {
			System.out.println(file.getName() + "不存在！");
			return null;
		}
		Map<String, Integer> hw = new HashMap<String, Integer>();
		InputStream in = null;
		...
		try {
			...
			hw.put("heigth", toInt(seg, 4));
			hw.put("width", toInt(seg, 6));
	 ...
	 */
	public final static void scaleWithTimes(String srcImageFile, String result, int scale, boolean flag) {
		try {
	 ...
	 */
	public final static byte[] scaleWithWH(InputStream srcImageFile, int width, int height, boolean bb) {
		try {
	 ...
	 *            比例不对时是否需要补白：true为补白; false为不补白;
	 */
	public final static void scaleWithWH(String srcImageFile, String result, int width, int height, boolean bb) {
		try {
			double ratio = 0.0; // 缩放比例
	 ...
	 *            目标切片高度
	 */
	public final static void cutWithStartXYWH(String srcImageFile, String result, int x, int y, int width, int height) {
		try {
			// 读取源图像
	 ...
	 */
	public final static void cutWithRowsAndCols(String srcImageFile, String descDir, int rows, int cols) {
		try {
			if (rows <= 0 || rows > 20)
				rows = 2; // 切片行数
			if (cols <= 0 || cols > 20)
	 ...
	 *            目标切片高度。默认150
	 */
	public final static void cutWithSetWH(String srcImageFile, String descDir, int destWidth, int destHeight) {
		try {
			if (destWidth <= 0)
				destWidth = 200; // 切片宽度
			if (destHeight <= 0)
	 ...
	 *            目标图像地址
	 */
	public final static void convertImgType(String srcImageFile, String formatName, String destImageFile) {
		try {
			File f = new File(srcImageFile);
			f.canRead();
			f.canWrite();
	 ...
	 *            目标图像地址
	 */
	public final static void colorfulToGray(String srcImageFile, String destImageFile) {
		try {
	 ...
	 */
	public final static void pressText(String pressText, String srcImageFile, String destImageFile, Font font, Color color, int location, float alpha) {
		try {
			...
			case 2:
				y = font.getSize();
				x = width - (getLength(pressText) * font.getSize()) - font.getSize();
				break;
			case 3:
			...
			case 4:
				...
				break;
			case 5:
				x = (width - (getLength(pressText) * font.getSize())) / 2;
			...
			g.dispose();
			if (null == destImageFile || "".equals(destImageFile)) {// 如果目标文件名称没有的话，就直接覆盖原文件
	 ...
	 * @param alpha 透明度 
	 */
	public static void pressImage(String pressimg, String targetimg, String destImageFile, int location, float alpha) {
		try {
			// 读取目标图片
	...
	public static void main(String[] args) {
		// 1-缩放图像：
		// 方法一：按比例缩放
		// ImageUtil.scale("e:/1.jpg", "e:/scaleSm.jpg", 2, false);// 测试OK
		// // 方法二：按高度和宽度缩放
		// ImageUtil.scaleWithWH("e:/1.jpg", "e:/scale1.jpg", 500, 300, true);// 测试OK
		// // 2-切割图像：
		// // 方法一：按指定起点坐标和宽高切割
		// ImageUtil.cutWithStartXYWH("e:/1.jpg", "e:/cut1.jpg", 0, 0, 200, 200);// 测试OK
		// // 方法二：指定切片的行数和列数
		// ImageUtil.cutWithRowsAndCols("e:/1.jpg", "e:/", 2, 2);// 测试OK
		// // 方法三：指定切片的宽度和高度
		// ImageUtil.cut3("e:/1.jpg", "e:/", 300, 300);// 测试OK
		// // 3-图像类型转换：
		// ImageUtil.convertImgType("e:/1.jpg", ImageUtil.IMAGE_TYPE_PNG, "e:/abc_convert.jpeg");// 测试OK
		// // 4-彩色转黑白：
		// ImageUtil.colorfulToGray("e:/1.jpg", "e:/abc_gray.jpg");// 测试OK
		// // 5-给图片添加文字水印：
		// // 方法一：
		// Font font=new Font( "宋体", Font.BOLD, 20);
		// ImageUtil.pressText("我是水印文字", "e:/1.jpg", "e:/abc_pressText.jpg", font,Color.green, 3, 0.5f);// 测试OK
		// // 方法二：
		// ImageUtil.pressText("我也是水印文字", "e:/1.jpg", "e:/abc_pressText2.jpg", "黑体", 36, Color.white, 80, 0, 0, 0.5f);// 测试OK
		//
		// // 6-给图片添加图片水印：
		// ImageUtil.pressImage("e:/cut1.jpg", "e:/1.jpg", "e:/abc_pressImage.jpg", 0, 0, 0.5f);// 测试OK
		ImageUtil.pressImage("e:/logo.png", "e:/1.jpg", "e:/abc_pressImage.jpg", 4, 0.8f);// 测试OK
	}
}
